...
$sql = "SELECT LBU02id, CaseReference, ExhibitRef from lbu02 WHERE Identifier = ? AND EvidenceID = ?";
$stmt = $connection->prepare($sql);
$stmt->bind_param("ss", $identifier, $evidenceID);  
$stmt->execute();
$stmt->bind_result($LBU02id, $caseReference, $exhibitReference);
$stmt->fetch();
mysqli_stmt_close($stmt);

// Audit Log
$action = "Viewed an LBU02 form. Case Reference: " . $caseReference . ". Case ID: " . $identifier . ". Exhibit Reference: " . $exhibitReference . ". Exhibit ID: " . $evidenceID . ". LBU02 ID: " . $LBU02id . ".";
$type = "Exhibit";
...
$query = "INSERT INTO auditlog 
    (Identifier, CaseReference, ExhibitReference, EvidenceID, EntryType, LBU02id, Timestamp, ActionerFullName, ActionerUsername, Action)
    VALUES
    (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = mysqli_prepare($connection, $query);
mysqli_stmt_bind_param($stmt, "ssssssssss", $identifier, $caseReference, $exhibitReference, $evidenceID, $type, $LBU02id, $timestamp, $fullName, $username, $action);
                ...
                while ($row = mysqli_fetch_assoc($results)) {
                    if ($row['ExternalBoolean'] === "true") {
                        ...
                        echo "<tr><td rowspan='2' style='font-size: 50px; font-weight: bold; border: 2px solid #5AAAFF; background-color: #5AAAFF; color: white;'>DFCMS</td> 
                                <td style='text-align: right; border: 2px solid #5AAAFF; background-color: #5AAAFF; color: white; font-weight: bold; font-size: 20px;'>" . 'LBU02 - Exhibit Dispatch Form' . "</td></tr>"; 
                        ...
                        echo "<tr><td class='lbu-high'>External Email Recipient</td><td>" . $row['ExternalEmail'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>Timestamp Sent</td><td>" . $row['EmailTimestamp'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>PDF MD5 Hash</td><td>" . $row['PDFMD5Hash'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>PDF SHA-1 Hash</td><td>" . $row['PDFSHA1Hash'] . "</td></tr>";
                        echo "</table>";
                        echo "<br/>";
                        ...
                        echo "<table class='styled-table' border='1' cellpadding='10' cellspacing='0' style='width: 100%;'>";
                        echo "<tr><td class='lbu-high'>Dispatched By Rank</td><td>" . $row['DispatchedByRank'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>Dispatched By Name</td><td>" . $row['DispatchedByName'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>Dispatched By Company</td><td>" . $row['DispatchedByCompany'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>Dispatched By Timestamp</td><td>" . $row['DispatchedByTime'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>Dispatched By Signature</td><td><img src='" . $row['DispatchedBySig'] . "' alt='Signature'></td></tr>";
                        echo "</table>";
                        echo "<br/>";

                        echo "<table class='styled-table' border='1' cellpadding='10' cellspacing='0' style='width: 100%;'>";
                        echo "<tr><td class='lbu-high'>Received By Rank</td><td>" . $row['ReceivedByRank'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>Received By Name</td><td>" . $row['ReceivedByName'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>Received By Company</td><td>" . $row['ReceivedByCompany'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>Received By Timestamp</td><td>" . $row['ReceivedByTime'] . "</td></tr>";
                        echo "<tr><td class='lbu-high'>Received By Signature</td><td><img src='" . $row['ReceivedBySig'] . "' alt='Signature'></td></tr>";
                        echo "</table>";
                        echo "<br/>";
